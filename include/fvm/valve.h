...
class valveMynard
{
 public:
  ...
  /*
    Valve model presented in Mynard & Smolich (2015) and therein cited references
    NB:
    - UNITS: cm/s/g
   */
  ...
  // output
  // methods
  void getaeff(double qLoc);
  void getb();
  void getl();
  double getStateDiff(double zetaLoc);
  double getFlowDiff(double qLoc, double zetaLoc);
  void getDiffImp(double *x, double *dxdt);
  void getFuncImp(double *x, double *f);
  void getFuncIncImp(double *x, double *f, double *dx);
  void evolveState();
  void evolveFlow();
  void update();
  void output();
