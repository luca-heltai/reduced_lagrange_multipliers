 ...
 * (A Albanese, L Cheng, M Ursino & N. W. Chbat, American Journal of
 * Physiology-Heart and Circulatory Physiology 2016 310, H899-H921)
 *
...
public:
   ...
   * - double dvpldt: dt volume in the pleural cavity (associated to cw
   * compliance)
   */
  ...
  double tau;
  /// local time variable that denotes the "position" within the respiratory
  /// cycle and is used when defining pmus
  double timeloc;
  ...
  // methods
  /**
   *  Read lung mechanics parameters from file (passed as input) through the
   * readLungMechParameters function and define the state variable vector
   * through the getState function
   */
  void
  ...
  /**
   * Default parameter values from Albanese et al (2015). If a variable is not
   * found under [category], then a default value is assigned. Time variables
   * are assigned in the main. Valid categories currently include methods,
   * compliances, unstressed volumes, resistances, respiratory params. Initial
   * conditions are computed assuming that at time t=0 (end-exhalation time),
   * all of the pressures in the lungs equilibrate to Patm = 0 cmH2O, whereas
   * Ppl has a subatmospheric value of -5 cmH2O.
   * @param ifile Input file containing parameters required to run simulations
   */
  void
  readLungMechParameters(string _file);
