...
public:
  ...
  // input
  string
    radiusMidFile; // directory in which the RadiusEnd-RadiusIni file is saved
  string displacementHdf5;    // directory in which the RadiusEnd-RadiusIni
                              // HDF5file is saved
  ...
  int coupling =
    0; // if 1 write file with RadiusEnd-RadiusIni for 1d-3d coupling
  ...
  solvePseudo3D();
  void
         setAreaIni(); // set // area at mid point vessel, before solveTimeStep
  double omega;
  double pBase; // external pressure at the beginning, if not specified
                // otherwise it's 0
  std::vector<double> new_displacement;
  std::vector<double>
  get_new_displacement_for_coupling();
  ...
  void
  compute_new_displacement_for_coupling(unsigned int);

  void
       printCouplingParameters();
  void readCouplingParameters(string);
  void
  init(string ifile);
  void
  saveFullState(int i) override;
};
