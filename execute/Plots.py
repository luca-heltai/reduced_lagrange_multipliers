...
cases=[#"uncoupled_0_elastic_NCELL1", 
        ...
        "uncoupled_elastic",
        "liver_elastic",
        "pseudo_liver_elastic_k8"
        #"pseudo_brain_elastic_k8",
        #"brain_elastic"
        ]
...
for testcase in cases:
    ...
    axs[0,0].plot(PExt_file[:,0], PExt_file[:,1]/mmHg, label=testcase)
    axs[0,0].set_title('External Pressure pe')
    axs[0,0].set_xlabel("time [s]")
    axs[0,0].set_ylabel(r"External Pressure [mmHg]")      
...
#axs[1,0].legend()
    ...
    axs[1, 1].plot(Flux_file[:,0], Flux_file[:,1])
    #axs[1, 1].vlines(np.arange(0.1, 1, 0.1), np.min(Flux_file[:,1]), np.max(Flux_file[:,1]), colors = 'black', linewidth = 0.5)
    axs[1, 1].set_title('Flow')
    axs[1,1].set_xlabel("time [s]")
    axs[1,1].set_ylabel("flow [ml/s]")
...
plt.show()
#tikzplotlib.save("/home/camilla/Desktop/latex/Latex/ECCOMAS24/pres/tikzsource/brain_half.tikz")
